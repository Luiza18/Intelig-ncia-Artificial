...
class Tela:
    def __init__(self):
        ...
        self.__busca = ['Profundidade', 'Amplitude', 'Custo uniforme', 'A*', 'Greedy']

    def desenhar_labirinto(self):
        layout = [
            [sg.Text('Selecione o método de busca:'), sg.Combo(self.__busca, default_value='', key='-METODO-', readonly=True),
            ...
            [sg.Multiline(default_text='\n'.join([' '.join(map(str, linha)) for linha in self.__labirinto.get_mapa()]),
             ...
             sg.Multiline(default_text='', size=(40, 20), font=('Courier New', 12), disabled=True, no_scrollbar=True,
                          key='-RESULT-', visible=False)]
        ]
        ...
        while True:
            ...
            if event == '-CRIAR_MAPA-':
                self.__labirinto = Labirinto()  # Criar um novo Labirinto
                window['-MAPA-'].update(visible=True,
                                        value='\n'.join([' '.join(map(str, linha)) for linha in self.__labirinto.get_mapa()]))
